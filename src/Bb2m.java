...
public class Bb2m extends JFrame {
	...
	public static void main(final String[] args) {
		...
		for (int n = 0; n < procs; n++) {
			final Thread t = new Thread("Worker-" + n) {
				@Override
				public void run() {
					while (true) {
						try {
							final Runnable r = q.take();
							r.run();
	...
	private void calc() {
		...
		for (int n = 0; n < images.length; n++) {
			for (int m = 0; m < images[n].length; m++) {
				...
				// don't use gray, it doesn't look as nice
				final BufferedImage im = (BufferedImage) createImage(iw, ih);
//				final BufferedImage im = new BufferedImage(iw, ih, BufferedImage.TYPE_BYTE_GRAY);
				l.add(new Runnable() {
					@Override
					public void run() {
						// no field accesses
